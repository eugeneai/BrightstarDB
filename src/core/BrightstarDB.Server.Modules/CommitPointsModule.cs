        ...
        public CommitPointsModule(IBrightstarService brightstarService, AbstractStorePermissionsProvider permissionsProvider)
        {
            ...
            Get("/{storeName}/commits", parameters =>
            {
                ...
                if (earliest != default(DateTime) && latest != default(DateTime))
                {
                    ...
                    var resourceUri = $"commits?latest={latest:s}&earliest={earliest:s}";
                    return Negotiate.WithPagedList(request, results.Select(MakeResponseObject), skip, take,
                        DefaultPageSize, resourceUri);
                }
                IEnumerable<ICommitPointInfo> commitPointInfos =
                    brightstarService.GetCommitPoints(parameters["storeName"], skip, take + 1);
                return Negotiate.WithPagedList(request, commitPointInfos.Select(MakeResponseObject), skip, take,
                    DefaultPageSize, "commits");
            });
